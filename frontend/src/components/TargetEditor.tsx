...
const TargetEditor = ({
  ...
  useEffect(() => {
    if (initialTargets) {
      ...
      setTargetProtein(initialTargets.protein_g?.toString() || "120");
      setTargetFat(initialTargets.fat_g?.toString() || "70");
    }
  }, [initialTargets]);
  ...
  const handleSaveTargets = async (e: React.FormEvent) => {
    ...
    try {
      const updatedTargets = await apiFetch(
        `/api/clinicians/patients/${patientId}/targets`,
        {
          method: "PUT",
          json: {
            ...
            protein_g: parseFloat(targetProtein) || 0,
            fat_g: parseFloat(targetFat) || 0,
          },
        },
      );
      if (updatedTargets) {
      ...
      <form onSubmit={handleSaveTargets} className="space-y-4">
        <div>
          ...
          <Input
            type="number"
            value={targetSodium}
            onChange={(e) => setTargetSodium(e.target.value)}
          />
        </div>
        <div>
          <label className="text-xs font-bold text-on-surface-variant block mb-1">
        ...
        <div>
          ...
          <Input
            ...
            onChange={(e) => setTargetPotassium(e.target.value)}
          />
        </div>
        <div>
          <label className="text-xs font-bold text-on-surface-variant block mb-1">
        ...
        <Button
          ...
          className="w-full bg-primary hover:bg-primary/95 text-white flex items-center justify-center gap-2 rounded-xl mt-2"
        >
