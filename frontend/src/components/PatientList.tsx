      ...
      <div className="bg-white border border-outline-variant rounded-xl flex flex-col">
        <div className="p-4 border-b border-outline-variant flex justify-between items-center bg-surface-bright/50 rounded-t-xl">
          <div>
            ...
            <p className="text-sm text-on-surface-variant">Select a patient to view their charts, logs, and modify profiles.</p>
          </div>
                  ...
                  <tr key={patient.id} className="hover:bg-surface-container-lowest transition-colors">
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        <div className="w-8 h-8 rounded-full bg-secondary-container flex items-center justify-center text-xs font-bold text-primary flex-shrink-0">{patInitials}</div>
                    ...
                    <td className="px-6 py-4 text-right">
                      <Button onClick={() => onSelectPatient(patient)} size="sm" className="bg-secondary-container text-on-surface hover:bg-secondary-container/80 text-xs font-bold px-3">
                        View Profile & Logs
                      </Button>
                    </td>
                  </tr>
      ...
      {isAddPatientOpen && (
        <div className="fixed inset-0 z-[100] bg-black/50 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white border border-outline-variant max-w-md w-full rounded-2xl shadow-xl p-6 relative">
            <button onClick={() => setIsAddPatientOpen(false)} className="absolute top-4 right-4 text-on-surface-variant hover:text-on-surface p-1 rounded-full hover:bg-outline-variant/20 border-none bg-transparent cursor-pointer">
