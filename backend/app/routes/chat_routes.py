...
from app.models import User, UserRole, DietaryTargets, FoodLogs
...
# Optional: Initialize ChromaDB if needed for external clinical guidelines
# chroma_client = chromadb.PersistentClient(path="./chroma_db")
# collection = chroma_client.get_or_create_collection(name="nutritional_guidelines")
...
def chat_with_nutrigabay(
    ...
    statement_targets = select(DietaryTargets).where(DietaryTargets.patient_id == current_user.id)
    targets = session.exec(statement_targets).first()
    
    # 2. Fetch Recent Food Logs (Today's logs)
    statement_logs = select(FoodLogs).where(FoodLogs.patient_id == current_user.id).order_by(FoodLogs.logged_at.desc()).limit(10)
