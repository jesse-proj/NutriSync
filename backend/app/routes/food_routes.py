...
from app.services.vision import GroqVisionProvider
...
async def analyze_food_photo(
    file: UploadFile = File(...), current_user: User = Depends(get_current_patient)
):
    ...
    if not dish_description or len(dish_description) < 3:
        raise HTTPException(
            status_code=400, detail="Could not identify food in the image."
        )
